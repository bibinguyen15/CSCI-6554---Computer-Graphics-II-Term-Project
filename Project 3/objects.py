...
class Object:
    def __init__(self, file, V, pRef, i, mode=-1):
        ...
        self.polygons = []
        self.vertices = []
        #self.color = np.array([0.9, 0.7, 0.7])
    ...
    def setVertices(self):
        ...
        for v in self.vertices:
            ...
            for i, p in enumerate(self.rawPolys):
                ...
                #else:
                    #print("Polygon:", p, "not used.")
            #print(normal)

            normal = unitVector(normal)

            #print("\nVertex:", v.number, "=", normal)
            v.normal = normal
            v.color *= self.i.phongIllumination(v.normal, 15)
            print(
    ...
    def setEdgesPolys(self):
        ...
        for p in self.polygons:
            ...
            for i in range(len(p.vertices)):
                ...
                #Add the vertices in order of descending y value
                if v1[1] > v2[1]:
        ...
        for p in self.polygons:
            ...
            while yScan < p.ymax and flag:
                ...
                for i in range(len(AET) // 2):
                    ...
                    #Initialize l values
                    I1 = start.vertices[1].color
                    I2 = start.vertices[0].color
                    I3 = end.vertices[0].color

                    Ia = I1 * (yScan - start.ymin) / (start.ymax - start.ymin) + \
                        I2 * (start.ymax - yScan) / (start.ymax - start.ymin)

                    temp = max(start.ymax - end.ymin, 1)

                    Ib = I1 * (yScan - end.ymin) / temp + \
                        I3 * (start.ymax - yScan) / temp
    ...
    def scanConversionPhong(self, image, depth):
        def printAET(AET):
            ...
            for e in AET:
                print("Edge:", e.name)
                print("Vertices in edge:", [v.number for v in e.vertices])
        ...
        for p in self.polygons:
            ...
            while yScan < p.ymax and flag:
                ...
                for i in range(len(AET) // 2):
                    ...
                    #Initialize l values
                    n1 = start.vertices[0].normal
                    n2 = start.vertices[1].normal
                    n3 = end.vertices[1].normal
                    #print(n1, n2, n3)

                    na = n1 * (yScan - start.ymin) / (start.ymax - start.ymin) + \
                        n2 * (start.ymax - yScan) / (start.ymax - start.ymin)

                    deno = max(start.ymax - end.ymin, 1)

                    nb = n1 * (yScan - end.ymin) / deno + \
                        n3 * (start.ymax - yScan) / deno
                    ...
                    for x in range(int(start.xmin), int(end.xmin)):
                        ...
                        if changeBack(z, 1) < depth[x][yScan]:
                            depth[x][yScan] = changeBack(z, 1)
                            color *= self.i.phongIllumination(np, 10)
                            image[x][yScan] = color
    ...
    def color(self, image, depth):
        n = 50 #it's not changing anything
        #Constant shading
        if self.mode == -1:
            print(f'In constant shading mode.')
            for p in self.polygons:
                p.color *= self.i.phongIllumination(p.normal, n)
